...
export const setUserTypeActionCreator = (user, rememberMe) => { //band or regular user
    return dispatch => {
        let queryParams = `?orderBy="email"&equalTo="${user.email}"&limitToFirst=1`;
...
export const signUpActionCreator = (newUser) => {
    return dispatch => {
        ...
        axios.post('https://identitytoolkit.googleapis.com/v1/accounts:signUp?key=AIzaSyBEV9Po-0nHkEKjOwJYeMEJVdA-E7xUtdM', authData)
            .then(response => {
...
export const signInActionCreator = (userData, rememberMe) => {
    return dispatch => {
        ...
        axios.post('https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=AIzaSyBEV9Po-0nHkEKjOwJYeMEJVdA-E7xUtdM', authData)
            .then(response => {
...
export const refreshTokenActionCreator = (token) => {
    return dispatch => {
        axios.post('https://securetoken.googleapis.com/v1/token?key=AIzaSyBEV9Po-0nHkEKjOwJYeMEJVdA-E7xUtdM', token)
            .then(response => {
